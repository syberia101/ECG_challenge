...
class read_DBLP_dump(object):
    ...
    def context_iter(self,dblp_path):
    ...
    def extract_feature(self,elem, features, include_key=False):
        ...
        for sub in elem:
            ...
            if sub.tag == 'title':
                ...
                text = self.count_pages(sub.text)
            else:
                text = sub.text
    ...
    def parse_all(self,dblp_path, save_path, include_key=False):
        self.log_msg("PROCESS: Start parsing...")
        f = open(save_path, 'w', encoding='utf8')
        for _, elem in self.context_iter(dblp_path):
            if elem.tag in self.all_elements:
                attrib_values = self.extract_feature(elem, self.all_features, include_key)
                f.write(str(attrib_values) + '\n')
            self.clear_element(elem)
        f.close()
        self.log_msg("FINISHED...")  # load the saved results line by line using json


    def parse_entity(self,dblp_path, save_path, conf_name, type_name, features=None, save_to_csv=False, include_key=False):
        """Parse specific elements according to the given type name and features"""
        self.log_msg("PROCESS: Start parsing for {}...".format(str(type_name)))
        ...
        for _, elem in self.context_iter(dblp_path):
            if elem.tag in type_name:
                ...
                if conf_name in str(elem.attrib['key']):
                #if 'topnoc' in elem.attrib :
                    print('element text')
                    print(elem.attrib)

                    attrib_values = self.extract_feature(elem, features, include_key)  # extract required features
                    ...
                    if cnt == len(features):
                        ...
                        part_entity += 1
                elif elem.tag not in self.all_elements:
                    continue
                self.clear_element(elem)
        if save_to_csv:
    ...
    def parse_author(self,dblp_path, save_path, save_to_csv=False):
    ...
    def parse_article(self,dblp_path, save_path, save_to_csv=False, include_key=False):
        type_name = ['article']
        features = ['title', 'author', 'year', 'journal', 'pages','inproceedings']
        info = self.parse_entity(dblp_path, save_path, type_name, features, save_to_csv=save_to_csv, include_key=include_key)
        self.log_msg('Total articles found: {}, articles contain all features: {}, articles contain part of features: {}'
                .format(info[0] + info[1], info[0], info[1]))
        self.log_msg("Features information: {}".format(str(info[2])))


    def parse_proceedings(self,dblp_path, save_path, conf_name, save_to_csv=False, include_key=False ):
        type_name = ["inproceedings", "proceedings"]
        features = ['title', 'author', 'year', 'pages', 'booktitle','pages','venue','ee','doi','key']
        info = self.parse_entity(dblp_path, save_path, conf_name, type_name, features, save_to_csv=save_to_csv, include_key=include_key)
        self.log_msg('Total proceedings found: {}, proceedings contain all features: {}, proceedings contain part of '
                'features: {}'.format(info[0] + info[1], info[0], info[1]))
        self.log_msg("Features information: {}".format(str(info[2])))


    def parse_book(self,dblp_path, save_path, save_to_csv=False, include_key=False):
        type_name = ["book"]
        features = ['title', 'author', 'publisher', 'isbn', 'year', 'pages']
        info = self.parse_entity(dblp_path, save_path, type_name, features, save_to_csv=save_to_csv, include_key=include_key)
        self.log_msg('Total books found: {}, books contain all features: {}, books contain part of features: {}'
                .format(info[0] + info[1], info[0], info[1]))
        self.log_msg("Features information: {}".format(str(info[2])))


    def parse_publications(self,dblp_path, save_path, save_to_csv=False, include_key=False):
        type_name = ['article', 'book', 'incollection', 'inproceedings']
        features = ['title', 'year', 'pages']
        info = self.parse_entity(dblp_path, save_path, type_name, features, save_to_csv=save_to_csv, include_key=include_key)
        self.log_msg('Total publications found: {}, publications contain all features: {}, publications contain part of '
                'features: {}'.format(info[0] + info[1], info[0], info[1]))
        self.log_msg("Features information: {}".format(str(info[2])))


    def parse_dblp(self,dblp_dump,conf_name,save_name):

        #conf_name='conf/icse/'
        dblp_path = self.root_path+dblp_dump
        save_path = self.root_path+save_name+'.json'
                    #'procedessing_icse.json'
        try:
            self.context_iter(dblp_path)
            self.log_msg("LOG: Successfully loaded \"{}\".".format(dblp_path))
        except IOError:
            self.log_msg("ERROR: Failed to load file \"{}\". Please check your XML and DTD files.".format(dblp_path))
            exit()
        self.parse_proceedings(dblp_path, save_path, conf_name, save_to_csv=False)
